...
const validationFreezeRelativePath = "docs/research/evidence/local-blind-routing-validation-freeze-attempt-2-0.4-alpha-round-11.json";
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-local-blind-round-11-validation-"));
  ...
  try {
    ...
    report = {
      ...
      evidenceClass: "local-hash-frozen-candidate-held-out-paired-static-routing",
      publicPreregistration: false,
      claimBoundary: "First paired static-routing observation for the locally frozen 0.4 candidate and its pre-repair baseline on eight task-coherent, real-history Round 11 targets selected before any Palace result was observed. It does not execute target tests or an Agent and cannot support Agent correctness, end-to-end reported Token, Agent tool-call, or Agent wall-time claims.",
      validityFailures,
    ...
    if (process.env.KEEP_LOCAL_BLIND_ROUND_11_VALIDATION_TEMP === "1") {
      process.stderr.write(`Keeping Round 11 validation workspaces at ${temporaryRoot}\n`);
    } else {
