...
const baselineRelativePath = "docs/research/evidence/disclosed-routing-round-7-after-constrained-module-pair-repair-0.4-alpha.json";
...
const studyId = "disclosed-routing-round-7-after-independent-anchor-confidence-cap-0.4-alpha";
...
const previousCandidateCommit = "228c3bde47f6930023496fdd0a54d43dba10091f";
...
const baselineSha256 = "7FBD82D10A99C65D4817349AD5E91C7A7237A712DADECD80E5707DBCA0386252";
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(
    path.join(os.tmpdir(), "vertex-palace-disclosed-routing-round-7-independent-anchor-confidence-")
  );
  ...
  try {
    ...
    report = {
      ...
      failures,
      claimBoundary: "Post-failure static routing regression on the eight disclosed Round 7 tasks after capping confidence when a high-confidence structural pair lacks an independently task-anchored implementation. It can measure calibration changes on known failures, but cannot establish held-out generalization or end-to-end Agent correctness, Token, tool-call, or wall-time gains.",
      heldOutAgainstCandidate: false,
      evidenceClass: "seen-development-regression",
      candidate: {
        productCommit: candidateCommit,
        repair: "independent-implementation-anchor-confidence-cap-v1",
    ...
    if (process.env.KEEP_DISCLOSED_ROUTING_ROUND_7_INDEPENDENT_ANCHOR_CONFIDENCE_TEMP === "1") {
