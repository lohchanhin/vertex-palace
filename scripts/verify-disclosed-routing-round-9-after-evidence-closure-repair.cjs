...
const manifestRelativePath = "docs/research/evidence/local-blind-routing-target-manifest-0.4-alpha-round-9.json";
const originalRelativePath = "docs/research/evidence/local-blind-routing-validation-0.4-alpha-round-9-attempt-2.json";
const invalidAttemptRelativePath = "docs/research/evidence/disclosed-routing-round-9-after-evidence-closure-repair-attempt-1-invalid-harness-0.4-alpha.json";
const previousDisclosedRelativePath = "docs/research/evidence/disclosed-routing-round-9-after-evidence-closure-repair-attempt-2-0.4-alpha.json";
...
const invalidAttemptPath = path.join(projectRoot, invalidAttemptRelativePath);
const previousDisclosedPath = path.join(projectRoot, previousDisclosedRelativePath);
const outputPath = require.main === module ? outputArgument(process.argv.slice(2)) : null;
const studyId = "disclosed-routing-round-9-after-evidence-closure-repair-0.4-alpha";
const targetCount = 8;
const repetitions = 2;
const budget = 6_000;
const routeLimit = 10;
...
async function main() {
  const [manifest, original, previousDisclosed] = await Promise.all([
    readJson(manifestPath),
    readJson(originalPath),
    readJson(previousDisclosedPath)
  ]);
  assert.equal(manifest.targets.length, targetCount);
  assert.equal(original.candidateGateStatus, "failed");
  assert.equal(original.status, "completed");
  assert.equal(previousDisclosed.gateStatus, "passed");

  const originalSha256 = await sha256File(originalPath);
  const invalidAttemptSha256 = await sha256File(invalidAttemptPath);
  const previousDisclosedSha256 = await sha256File(previousDisclosedPath);
  const manifestSha256 = await sha256File(manifestPath);
  const candidateCliSha256 = await sha256File(candidateCliPath);
  const candidateSourceTreeBefore = await hashSourceTree();
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-round-9-disclosed-repair-"));
  const targets = [];
  ...
  try {
    for (const target of manifest.targets) {
      ...
      assert.ok(originalTarget, `Original Round 9 target is missing: ${target.name}`);
    ...
    const report = {
      ...
      claimBoundary: "Disclosed regression on the eight already observed Round 9 tasks after generic evidence-closure, scope, parser, and safe-mode repairs. The immutable formal Round 9 candidate gate remains failed. This result is not held out, does not execute target tests or an Agent, and supports no Agent correctness, Token, tool-call, or wall-time claim. A fresh Round 10 is required for new generalization evidence.",
      ...
      priorInvalidAttempt: {
        path: invalidAttemptRelativePath,
        sha256: invalidAttemptSha256,
        classification: "invalid-harness",
        reason: "The harness failed to strip line ranges such as :87-117 from context boundary paths, creating false evaluate/context route disagreements.",
        preservedWithoutModification: true
      },
      previousDisclosedAttempt: {
        path: previousDisclosedRelativePath,
        sha256: previousDisclosedSha256,
        status: previousDisclosed.status,
        gateStatus: previousDisclosed.gateStatus,
        aggregate: previousDisclosed.aggregate,
        supersededReason: "Attempt 2 passed the disclosed core gate but retained low-focus five-file routes for two targets; Attempt 3 measures subsequent generic module-mirror and additive external-contract repairs.",
        preservedWithoutModification: true
      },
      targetSelection: {
      ...
      protocol: {
        ...
        oraclePolicy: {
          ...
          fullChangedFiles: "Retained for comparability with the immutable formal Round 9 result."
        },
        gates: {
