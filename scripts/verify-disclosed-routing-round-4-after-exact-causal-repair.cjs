...
const originalObservationRelativePath =
...
const studyId =
  "disclosed-cross-repository-routing-round-4-after-exact-causal-repair-0.4-alpha";
const originalCandidateCommit = "efd53274e42fb8123745f2b8bb09a24e4fa384b7";
const originalCandidateCliSha256 =
  "E0A2F54C826E742DF03BA0BC965C89AF834B3BBA199C1E135E63E21B291011D0";
const originalObservationSha256 =
  "7B8E3833A71D60645DF134D8B87ADF49EAA5557EE59A6AB6D64A537C8A3BB5D3";
const candidateCommit = "352c66679c040863fd3aff1621519c2e5b3ec409";
const candidateCliSha256 =
  "14547029262CAD39D8495A13E669836A17AAD8860AF3D3368B35EF77741D921D";
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(
    path.join(os.tmpdir(), "vertex-palace-disclosed-routing-round-4-exact-causal-")
  );
  ...
  try {
    ...
    report = {
      ...
      failures,
      claimBoundary: "Disclosed development regression for candidate 352c666 on the eight Round 4 tasks whose failures were observed and used to develop this repair. This is not held-out or generalization evidence, does not execute target tests, and cannot support Agent correctness, Token, tool-call, or wall-time claims.",
      heldOutAgainstCandidate: false,
      evidenceClass: "disclosed-development-regression-after-held-out-failure",
      disclosure: {
        tasksUsedDuringCandidateDevelopment: true,
        originalCandidateCommit,
        originalCandidateCliSha256,
        originalObservationPath: originalObservationRelativePath,
    ...
    if (process.env.KEEP_DISCLOSED_ROUTING_ROUND_4_REPAIR_TEMP === "1") {
      process.stderr.write(`Keeping disclosed Round 4 repair repositories at ${temporaryRoot}\n`);
    } else {
...
async function assertFrozenInputs() {
  run("git", ["cat-file", "-e", `${originalCandidateCommit}^{commit}`], {
    cwd: projectRoot
  });
  ...
  assert.equal(await sha256File(originalObservationPath), originalObservationSha256);
...
function outputArgument(args) {
  ...
  assert.notEqual(resolved, manifestPath, "Output cannot overwrite the frozen target manifest.");
  assert.notEqual(
    resolved,
    originalObservationPath,
    "Output cannot overwrite the original Round 4 held-out observation."
  );
  return resolved;
}
