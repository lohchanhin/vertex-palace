...
const studyId = "local-blind-routing-validation-round-10-attempt-1-0.4-alpha";
const selectionStudyId = "local-blind-routing-round-10-0.4-alpha";
const manifestRelativePath = "docs/research/evidence/local-blind-routing-target-manifest-0.4-alpha-round-10.json";
const candidateFreezeRelativePath = "docs/research/evidence/local-blind-candidate-freeze-0.4-alpha-round-10.json";
const validationFreezeRelativePath = "docs/research/evidence/local-blind-routing-validation-freeze-0.4-alpha-round-10.json";
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-local-blind-round-10-validation-"));
  ...
  try {
    ...
    report = {
      ...
      evidenceClass: "local-hash-frozen-candidate-held-out-paired-static-routing",
      publicPreregistration: false,
      claimBoundary: "First paired static-routing observation for the locally frozen 0.4 candidate and pre-change baseline on eight mechanically selected Round 10 tasks. It does not execute target tests or an Agent and cannot support Agent correctness, end-to-end reported Token, Agent tool-call, or Agent wall-time claims.",
      validityFailures,
    ...
    if (process.env.KEEP_LOCAL_BLIND_ROUND_10_VALIDATION_TEMP === "1") {
      process.stderr.write(`Keeping Round 10 validation workspaces at ${temporaryRoot}\n`);
    } else {
...
async function assertFrozenInputs() {
  ...
  assert.equal(validationFreeze.protocol.noTaskContentOpenedBeforeValidatorFreeze, true);
