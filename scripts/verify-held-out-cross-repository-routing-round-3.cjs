...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-held-out-routing-round-3-"));
  assert.ok(
  ...
  try {
    ...
    report = {
      ...
      failures,
      claimBoundary: "First static routing observation on eight mechanically selected repositories and Git-history tasks unseen during candidate development. This is not an end-to-end Agent performance benchmark and cannot support Token or wall-time claims.",
    ...
    if (process.env.KEEP_HELD_OUT_ROUTING_ROUND_3_TEMP === "1") {
      process.stderr.write(`Kept round-3 held-out validation data at ${temporaryRoot}\n`);
    } else {
