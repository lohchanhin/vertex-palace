...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(
    path.join(os.tmpdir(), "vertex-palace-held-out-confidence-round-8-")
  );
  ...
  try {
    ...
    report = {
      ...
      claimBoundary: "First paired static-routing and confidence-calibration observation for baseline 228c3bd and candidate 1a02d89 on eight mechanically selected Round 8 tasks not used during candidate development. This candidate-held-out sample is not model-unseen evidence, does not execute target tests or an Agent, and cannot support Agent correctness, reported Token, tool-call, or wall-time claims.",
      heldOutAgainstCandidate: true,
      evidenceClass: "preregistered-candidate-held-out-paired-static-calibration",
      candidate: {
        productCommit: candidateCommit,
    ...
    if (process.env.KEEP_HELD_OUT_CONFIDENCE_ROUND_8_TEMP === "1") {
      process.stderr.write(`Keeping Round 8 validation repositories at ${temporaryRoot}\n`);
    } else {
...
async function assertFrozenInputs() {
  ...
  assert.equal(manifest.studyId, studyId);
...
async function cloneConditionTarget(target, sourceRoot, conditionRoot, targetContainer) {
  ...
  run("git", ["clone", "--quiet", "--shared", "--no-checkout", sourceRoot, conditionRoot], {
    cwd: targetContainer,
    timeout: 120_000
  });
  run("git", [
