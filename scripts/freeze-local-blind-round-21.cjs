...
const planRelativePath = "docs/research/evidence/local-blind-routing-repository-plan-0.4-stable-round-21.json";
const poolRelativePath = "docs/research/evidence/local-blind-routing-repository-pool-0.4-stable-round-21.json";
const headRelativePath = "docs/research/evidence/local-blind-routing-repository-head-verification-0.4-stable-round-21.json";
const freezeRelativePath = "docs/research/evidence/local-blind-candidate-freeze-0.4-stable-round-21.json";
const candidateVersion = "0.4.0";
...
const artifactPaths = [
  ...
  poolRelativePath,
  headRelativePath,
  "scripts/freeze-local-blind-repository-pool-round-21.cjs",
  "scripts/freeze-local-blind-round-21.cjs",
  "scripts/prepare-local-blind-routing-candidates-round-21.cjs",
  ...
  "scripts/test/round21-preregistration.test.cjs"
];
...
async function main() {
  ...
  const candidateProductCommit = researchCommit;
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-round21-product-freeze-"));
  let candidateInstall;
  let baselineInstall;
  try {
    candidateInstall = await packAndInstallLocalCandidate(temporaryRoot);
    baselineInstall = await installPublicPackage(temporaryRoot, "baseline", baselineVersion);
  } finally {
    await rm(temporaryRoot, { recursive: true, force: true });
  }
  assert.equal(candidateInstall.cliSha256, localCliSha256, "Packed stable-candidate CLI differs from the frozen local CLI");
  assert.equal(candidateInstall.mcpSha256, localMcpSha256, "Packed stable-candidate MCP differs from the frozen local MCP");
  ...
  const freeze = {
    ...
    claimBoundary: "The publicly committed 0.4 stable-candidate tarball, public 0.3 baseline, product artifacts, repository pool, selectors, validator, gates, and execution order were frozen before any Round 21 commit history, task, diff, oracle, semantic review, or Palace result was inspected.",
    candidate: {
      role: "publicly-committed-0.4-stable-candidate",
      package: `vertex-palace@${candidateVersion}`,
      version: candidateVersion,
      productCommit: candidateProductCommit,
      researchCommit,
      sourceState: "immutable-public-git-commit-matched-to-local-tarball",
      cliPath,
...
async function packAndInstallLocalCandidate(temporaryRoot) {
  const packRoot = path.join(temporaryRoot, "candidate-pack");
  await mkdir(packRoot, { recursive: true });
  const packed = JSON.parse(runNpm([
    "pack",
    "--json",
    "--pack-destination", packRoot
  ], { cwd: projectRoot, timeout: 300_000 }).stdout)[0];
  assert.equal(packed.version, candidateVersion);
  const tarballPath = path.join(packRoot, packed.filename);
  const installed = await installPackageSpec(
    temporaryRoot,
    "candidate",
    tarballPath,
    candidateVersion
...
async function installPackageSpec(temporaryRoot, name, spec, expectedVersion) {
  const root = path.join(temporaryRoot, name);
  await mkdir(root, { recursive: true });
  runNpm(["init", "-y"], { cwd: root });
  runNpm([
...
function outputArgument(args) {
  ...
  assert.equal(resolved, path.join(projectRoot, freezeRelativePath), "Round 21 freeze must use its canonical path");
  return resolved;
}
