...
const studyId = "disclosed-routing-regression-round-2-after-duplicate-module-repair-0.4-alpha";
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-disclosed-routing-round-2-after-duplicate-module-repair-"));
  assert.ok(
