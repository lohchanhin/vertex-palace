...
async function main(outputPath) {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-local-blind-round-11-queue-"));
  ...
  try {
    ...
    queue = {
      ...
      claimBoundary: "Mechanical candidate queue and hunk-addressed diff packets only. No Round 11 candidate task was sent to Palace, and no semantic target acceptance decision was made by this script.",
      ...
      selector: {
        path: "scripts/prepare-local-blind-routing-candidates-round-11.cjs",
        sha256: freeze.artifacts["scripts/prepare-local-blind-routing-candidates-round-11.cjs"]
      },
      rules: {
        ...pool.rules,
        fetchDepth,
    ...
    if (process.env.KEEP_LOCAL_BLIND_ROUND_11_QUEUE_TEMP === "1") {
      process.stderr.write(`Keeping Round 11 candidate repositories at ${temporaryRoot}\n`);
    } else {
...
async function preserveFailure(outputPath, error) {
  try {
    ...
    await writeFile(outputPath, `${JSON.stringify({
      ...
      claimBoundary: "Unexpected queue-preparation failure preserved before any Round 11 candidate task was sent to Palace.",
      palaceCallsOnCandidateTasks: 0,
      error: summarizeError(error)
