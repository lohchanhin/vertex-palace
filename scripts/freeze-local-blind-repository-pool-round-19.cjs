...
async function main() {
  ...
  assert.equal(
    ...
    "Prior exclusion source changed after the Round 19 plan was written"
  );
  ...
  assert.equal(
    ...
    "Round 19 prior-observation count differs from the frozen plan"
  );

  const plannedUrls = plan.repositoryPlan.map(({ url }) => normalizeRepositoryUrl(url));
  assert.equal(new Set(plannedUrls).size, plannedUrls.length, "Round 19 roster contains duplicate URLs");
  ...
  const pool = {
    ...
    claimBoundary: "A 48-entry URL-and-HEAD-only roster was queried before any Round 19 history, subject, diff, task, review, oracle, or Palace result was inspected. Definitively missing URLs were preserved and skipped mechanically. The canonical pool contains the first eight reachable repositories per language family in frozen order. Candidate source, selectors, and validation artifacts must still be frozen before target selection. This is not public preregistration.",
    ...
    priorExclusionSource: {
      ...
      includeRound18AttemptedRepositories: true,
...
async function writePoolFailure({
  ...
  const document = {
    ...
    claimBoundary: "Round 19 failed during the frozen URL-and-HEAD-only reachability phase. No canonical pool was written and no history, candidate task, diff, review, oracle, or Palace result was inspected. This is a protocol or environment result, not a Vertex Palace product result."
  };
...
function assertPlan(plan) {
  ...
  assert.equal(plan.priorExclusionSource.expectedUniqueRepositoryCount, 194);
  ...
  for (let index = 0; index < plan.repositoryPlan.length; index += 1) {
    ...
    assert.equal(
      ...
      plan.rules.requiredLanguageFamilies[index % plan.rules.requiredLanguageFamilies.length],
      "Repository roster is not interleaved by language family"
    );
