...
const studyId = "held-out-cross-repository-routing-round-6-0.4-alpha";
const poolRelativePath = "docs/research/evidence/held-out-routing-repository-pool-0.4-alpha-round-6.json";
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-held-out-round-6-selection-"));
  ...
  try {
    ...
    manifest = {
      ...
      selector: {
        ...
        path: "scripts/select-held-out-routing-targets-round-6.cjs"
      },
      repositoryPool: {
    ...
    if (process.env.KEEP_HELD_OUT_SELECTION_ROUND_6_TEMP === "1") {
      process.stderr.write(`Keeping Round 6 selection repositories at ${temporaryRoot}\n`);
    } else {
