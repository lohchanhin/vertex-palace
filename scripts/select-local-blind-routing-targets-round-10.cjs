...
const studyId = "local-blind-routing-round-10-0.4-alpha";
const poolRelativePath = "docs/research/evidence/local-blind-routing-repository-pool-0.4-alpha-round-10.json";
const freezeRelativePath = "docs/research/evidence/local-blind-candidate-freeze-0.4-alpha-round-10.json";
...
main().catch(async (error) => {
  ...
  try {
    ...
    await writeFile(outputPath, `${JSON.stringify({
      ...
      claimBoundary: "Unexpected local selection failure preserved before any Round 10 task was sent to Palace. This is not a product result or public preregistration.",
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-local-blind-round-10-selection-"));
  ...
  try {
    ...
    manifest = {
      ...
      generatedAt: new Date().toISOString(),
      status,
      claimBoundary: "Mechanical local candidate-held-out target selection only. The frozen baseline and candidate have not initialized, indexed, routed, packed, contextualized, or evaluated any selected Round 10 task. This locally hash-frozen evidence is not public preregistration.",
      ...
      selector: {
        path: "scripts/select-local-blind-routing-targets-round-10.cjs",
        sha256: freeze.artifacts["scripts/select-local-blind-routing-targets-round-10.cjs"]
      },
      repositoryPool: {
    ...
    if (process.env.KEEP_LOCAL_BLIND_ROUND_10_SELECTION_TEMP === "1") {
      process.stderr.write(`Keeping Round 10 selection repositories at ${temporaryRoot}\n`);
    } else {
...
async function validatePool(pool, freeze) {
  ...
  assert.equal(new Set(poolUrls).size, pool.repositoryPool.length, "Round 10 pool contains duplicate URLs");
  for (const repository of pool.repositoryPool) {
...
async function collectObservedRepositories(document, visited = new Set()) {
  ...
  if (visited.has(absolute)) throw new Error(`Recursive exclusion source: ${source.path}`);
