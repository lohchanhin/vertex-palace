...
const validationFreezeRelativePath = "docs/research/evidence/local-blind-routing-validation-freeze-attempt-2-0.4-alpha-round-19.json";
const resultRelativePath = "docs/research/evidence/local-blind-routing-validation-0.4-alpha-round-19-attempt-2.json";
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-local-blind-round-19-attempt-2-validation-"));
  ...
  try {
    ...
    report = {
      ...
      evidenceClass: "local-hash-frozen-candidate-held-out-paired-static-routing",
      publicPreregistration: false,
      claimBoundary: "Corrected paired static-routing replay for the unchanged locally frozen post-Round-15 evidence-closure candidate and baseline after a preserved commit-message-contract harness failure exposed seven target pairs. No product tuning, target removal, oracle change, gate change, or condition-order change occurred before this replay. It is not a pristine first observation for those seven targets, does not execute target tests or an Agent, and cannot support Agent correctness, end-to-end reported Token, Agent tool-call, or Agent wall-time claims.",
      validityFailures,
    ...
    if (process.env.KEEP_LOCAL_BLIND_ROUND_12_VALIDATION_TEMP === "1") {
      process.stderr.write(`Keeping Round 19 attempt-2 validation workspaces at ${temporaryRoot}\n`);
    } else {
...
async function assertFrozenInputs() {
  ...
  assert.equal(validationFreeze.protocol.noPalaceResultObservedBeforeValidatorFreeze, false);
  assert.equal(validationFreeze.protocol.palaceCallsOnSelectedTasksBeforeFreeze, 98);
  assert.equal(validationFreeze.protocol.pairedTargetsObservedBeforeFreeze, 7);
  assert.equal(validationFreeze.protocol.attempt1PartialObservationPreserved, true);
  assert.equal(validationFreeze.protocol.productTuningBeforeFreeze, false);
  assert.equal(validationFreeze.protocol.harnessCorrectionOnly, true);
...
function outputArgument(args) {
  ...
  assert.equal(resolved, path.join(projectRoot, resultRelativePath), "Round 19 attempt-2 result must use its canonical path");
  return resolved;
}
