...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-local-blind-round-19-validation-"));
  ...
  try {
    ...
    report = {
      ...
      evidenceClass: "local-hash-frozen-candidate-held-out-paired-static-routing",
      publicPreregistration: false,
      claimBoundary: "First paired static-routing observation for the locally frozen post-Round-15 evidence-closure candidate and its pre-optimization baseline on eight task-coherent, real-history Round 19 targets selected before any candidate task was sent to Palace. It does not execute target tests or an Agent and cannot support Agent correctness, end-to-end reported Token, Agent tool-call, or Agent wall-time claims.",
      validityFailures,
    ...
    if (process.env.KEEP_LOCAL_BLIND_ROUND_12_VALIDATION_TEMP === "1") {
      process.stderr.write(`Keeping Round 19 validation workspaces at ${temporaryRoot}\n`);
    } else {
