...
const { finalizeReviewedTargets } = require("./lib/round16-target-selection.cjs");
...
async function preserveFailure(outputPath, error) {
  try {
    ...
    await writeFile(outputPath, `${JSON.stringify({
      ...
      claimBoundary: "Unexpected target-finalization failure preserved before any Round 16 candidate task was sent to Palace.",
      palaceCallsOnCandidateTasks: 0,
