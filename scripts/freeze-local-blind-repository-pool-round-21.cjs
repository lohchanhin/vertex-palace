...
const planRelativePath =
  "docs/research/evidence/local-blind-routing-repository-plan-0.4-stable-round-21.json";
const poolRelativePath =
  "docs/research/evidence/local-blind-routing-repository-pool-0.4-stable-round-21.json";
const headEvidenceRelativePath =
  "docs/research/evidence/local-blind-routing-repository-head-verification-0.4-stable-round-21.json";
const failureEvidenceRelativePath =
  "docs/research/evidence/local-blind-routing-repository-pool-freeze-0.4-stable-round-21-attempt-1-failure.json";
...
async function main() {
  ...
  assert.equal(
    ...
    "Prior exclusion source changed after the Round 21 plan was written"
  );
  ...
  assert.equal(
    ...
    "Round 21 prior-observation count differs from the frozen plan"
  );

  const plannedUrls = plan.repositoryPlan.map(({ url }) => normalizeRepositoryUrl(url));
  assert.equal(new Set(plannedUrls).size, plannedUrls.length, "Round 21 roster contains duplicate URLs");
  ...
  const pool = {
    ...
    priorExclusionSource: {
      ...
      includeRound20CanonicalPoolAndPriorChain: true,
...
async function writePoolFailure({
  ...
  const document = {
    ...
    claimBoundary: "Round 21 failed during the frozen URL-and-HEAD-only reachability phase. No canonical pool was written and no history, candidate task, diff, review, oracle, or Palace result was inspected. This is a protocol or environment result, not a Vertex Palace product result."
  };
...
function assertPlan(plan) {
  ...
  assert.equal(plan.priorExclusionSource.expectedUniqueRepositoryCount, 258);
