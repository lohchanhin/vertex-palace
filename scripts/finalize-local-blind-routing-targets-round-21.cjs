...
const { finalizeReviewedTargets } = require("./lib/round21-target-selection.cjs");
...
async function main({ queuePath, reviewPath, outputPath }) {
  ...
  const manifest = {
    ...
    claimBoundary: "Pre-route whole-target semantic selection under the public Round 21 preregistration. Every selected target is the first coherent candidate in newest-first order for its repository and the first two accepted repositories in frozen language-family order. This is not a Palace product result.",
...
async function preserveFailure(outputPath, error) {
  try {
    ...
    await writeFile(outputPath, `${JSON.stringify({
      ...
      claimBoundary: "Unexpected target-finalization failure preserved before any Round 21 candidate task was sent to Palace.",
      palaceCallsOnCandidateTasks: 0,
