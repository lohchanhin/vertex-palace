...
const validationFreezeRelativePath = "docs/research/evidence/local-blind-routing-validation-freeze-0.4-stable-round-21.json";
const resultRelativePath = "docs/research/evidence/local-blind-routing-validation-0.4-stable-round-21.json";
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-local-blind-round-21-validation-"));
  ...
  try {
    ...
    if (process.env.KEEP_LOCAL_BLIND_ROUND_12_VALIDATION_TEMP === "1") {
      process.stderr.write(`Keeping Round 21 validation workspaces at ${temporaryRoot}\n`);
    } else {
...
function outputArgument(args) {
  ...
  assert.equal(resolved, path.join(projectRoot, resultRelativePath), "Round 21 result must use its canonical path");
  return resolved;
}
