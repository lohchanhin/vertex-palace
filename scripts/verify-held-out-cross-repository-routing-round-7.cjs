...
const studyId = "held-out-cross-repository-routing-round-7-0.4-alpha";
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(
    path.join(os.tmpdir(), "vertex-palace-held-out-routing-round-7-")
  );
  ...
  try {
    ...
    report = {
      ...
      failures,
      claimBoundary: "First static routing observation for candidate f612076 on eight mechanically selected Round 7 repository tasks not used during candidate development. This candidate-held-out sample is not model-unseen evidence, does not execute target tests, and cannot support Agent correctness, Token, tool-call, or wall-time claims.",
    ...
    if (process.env.KEEP_HELD_OUT_ROUTING_ROUND_7_TEMP === "1") {
      process.stderr.write(`Keeping Round 7 validation repositories at ${temporaryRoot}\n`);
    } else {
