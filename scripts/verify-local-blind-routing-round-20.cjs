...
const studyId = "local-blind-routing-validation-round-20-0.4-stable-candidate";
const selectionStudyId = "local-blind-routing-round-20-0.4-stable-candidate";
const manifestRelativePath = "docs/research/evidence/local-blind-routing-target-manifest-0.4-stable-round-20.json";
const candidateFreezeRelativePath = "docs/research/evidence/local-blind-candidate-freeze-0.4-stable-round-20.json";
const validationFreezeRelativePath = "docs/research/evidence/local-blind-routing-validation-freeze-0.4-stable-round-20.json";
const resultRelativePath = "docs/research/evidence/local-blind-routing-validation-0.4-stable-round-20.json";
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-local-blind-round-20-validation-"));
  ...
  try {
    ...
    const conditions = {
      ...
      candidate: {
        ...
        productCommit: candidateFreeze.candidate.productCommit,
    ...
    report = {
      ...
      evidenceClass: "local-hash-frozen-candidate-held-out-paired-static-routing",
      publicPreregistration: true,
      claimBoundary: "First paired static-routing observation for the publicly preregistered Round 20 targets, immutable public 0.4 alpha candidate, and public 0.3 stable baseline. No product tuning, target removal, oracle change, gate change, or condition-order change occurred before this result. It does not execute target tests or an Agent and cannot support Agent correctness, end-to-end reported Token, Agent tool-call, or Agent wall-time claims.",
      validityFailures,
      ...
      products: {
        ...
        candidate: {
          ...
          productCommit: conditions.candidate.productCommit,
    ...
    if (process.env.KEEP_LOCAL_BLIND_ROUND_12_VALIDATION_TEMP === "1") {
      process.stderr.write(`Keeping Round 20 validation workspaces at ${temporaryRoot}\n`);
    } else {
...
async function assertFrozenInputs() {
  ...
  assert.equal(validationFreeze.status, "publicly-frozen-validation");
  assert.equal(validationFreeze.freezeAttempt, 1);
  assert.equal(validationFreeze.publicPreregistration, true);
  ...
  assert.equal(candidateFreeze.status, "publicly-preregistered-product-freeze");
  assert.equal(candidateFreeze.publicPreregistration, true);
  run("git", ["cat-file", "-e", `${candidateFreeze.candidate.researchCommit}^{commit}`], { cwd: projectRoot });
  assert.equal(
  ...
  assert.equal(manifest.publicPreregistration, true);
...
async function buildBaselineCli(temporaryRoot, baseline) {
  ...
  runNpm(["init", "-y"], { cwd: baselineRoot });
  const install = runNpm([
    ...
    "--no-audit",
    "--no-fund",
    "--cache", path.join(baselineRoot, "npm-cache"),
    baseline.package
  ], { cwd: baselineRoot, timeout: 600_000 });
  const installElapsedMs = Math.round(performance.now() - installStartedAt);
  const packageRoot = path.join(baselineRoot, "node_modules", "vertex-palace");
  const packageJson = JSON.parse(await readFile(path.join(packageRoot, "package.json"), "utf8"));
  assert.equal(packageJson.version, baseline.version);
  const cliPath = path.join(packageRoot, baseline.cliPath);
...
async function materializeTarget(target, root, temporaryRoot) {
  const attempts = [];
  let lastError = null;
  let lastFailureCategory = "harness-contract";
  for (let attempt = 1; attempt <= materializationAttempts; attempt += 1) {
    ...
    try {
      ...
      lastError = error;
      lastFailureCategory = materializationFailureCategory(error);
      attempts.push({
        attempt,
        status: lastFailureCategory === "environment-or-setup"
          ? "environment-failed"
          : "harness-failed",
        elapsedMs: Math.round(performance.now() - startedAt),
      ...
      if (lastFailureCategory !== "environment-or-setup") break;
...
function verifyPinnedTarget(target, root) {
  ...
  assert.equal(
    firstNonemptyCommitMessageLine(
      run("git", ["show", "-s", "--format=%B", target.groundTruthCommit], { cwd: root }).stdout
    ),
    target.task
  );
...
function studyValidityFailures(targets, baselineBuild) {
  ...
  for (const target of targets) {
    ...
    if (target.failureCategory === "harness-contract") {
      failures.push(`${target.name}: target materialization harness contract failed`);
    }
...
function materializationFailureTarget(target, targetIndex, materialization) {
  return {
    ...
    materializationAttempts: materialization.attempts,
    status: "failed",
    failureCategory: materialization.failureCategory,
...
function firstNonemptyCommitMessageLine(message) {
...
function outputArgument(args) {
  ...
  assert.equal(resolved, path.join(projectRoot, resultRelativePath), "Round 20 result must use its canonical path");
  return resolved;
}
...
function materializationFailureCategory(error) {
  return isEnvironmentFailure(error) ? "environment-or-setup" : "harness-contract";
}
...
module.exports = {
  ...
  materializationFailureCategory,
  normalizePath
};
