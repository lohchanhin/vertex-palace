...
const candidateCommit = "b6ff88fc126800a799973447f5ce6b37b925a6a3";
const budget = 6_000;
const routeLimit = 9;
const maxDrawers = 4;
const repetitions = 2;
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-route-precision-cross-repo-"));
  assert.ok(
  ...
  try {
    ...
    report = {
      schemaVersion: 1,
      studyId: "route-precision-cross-repository-replication-0.4-alpha",
      generatedAt: new Date().toISOString(),
      claimBoundary: "Pinned cross-repository static routing replication only; not held-out and not an end-to-end Agent performance benchmark.",
  ...
  for (let trial = 1; trial <= repetitions; trial += 1) {
    ...
    try {
      ...
      trials.push({
        trial,
        status: "completed",
        evaluationCacheState: trial === 1 ? "cold-index" : "warm-index",
