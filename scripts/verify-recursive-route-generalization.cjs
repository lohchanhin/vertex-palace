...
const candidateCommit = "716a6c4e2abb6432d59a17db14a940e90918fe2b";
const budget = 6_000;
const routeLimit = 9;
const maxDrawers = 4;
const repetitions = 2;
...
const candidateTargets = [
  ...
  {
    name: "missing-artifact-family-confidence-cap",
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-recursive-route-generalization-"));
  assert.ok(
  ...
  try {
    ...
    report = {
      schemaVersion: 1,
      studyId: "recursive-route-generalization-regression-0.4-alpha",
      ...
      protocol: {
        ...
        candidateTargetSet: "previously observed artifact families, compound repairs, and one negative control",
        heldOut: false,
    ...
    if (process.env.KEEP_RECURSIVE_ROUTE_TEMP === "1") {
      process.stderr.write(`Kept recursive route validation data at ${temporaryRoot}\n`);
    } else {
