...
const originalRelativePath = "docs/research/evidence/local-blind-routing-validation-0.4-alpha-round-10-attempt-1.json";
...
const studyId = "disclosed-routing-round-10-after-generic-causal-repair-0.4-alpha";
const targetCount = 8;
const repetitions = 2;
const budget = 6_000;
const routeLimit = 10;
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-round-10-disclosed-repair-"));
  const targets = [];
  ...
  try {
    for (const target of manifest.targets) {
      ...
      const originalTarget = original.targets.find((entry) => entry.name === target.name);
        assert.ok(originalTarget, `Original Round 10 target is missing: ${target.name}`);
    ...
    const report = {
      ...
      claimBoundary: "Disclosed regression on the eight already observed Round 10 tasks after generic workspace-ownership, directed causal-import, transitive-test-impact, auxiliary-evidence, stopping, and metric-precision repairs. The immutable formal Round 10 candidate gate remains failed. This result is not held out, does not execute target tests or an Agent, and supports no Agent correctness, Token, tool-call, or wall-time claim. A fresh Round 11 is required for new generalization evidence.",
      ...
      protocol: {
        ...
        oraclePolicy: {
          ...
          fullChangedFiles: "Retained for comparability with the immutable formal Round 10 result."
        },
        gates: {
