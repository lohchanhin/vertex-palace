...
const commands = [
  {
    label: "round16-research-harness",
    command: "node",
    args: [
      "--test",
...
async function main() {
  ...
  const evidence = {
    ...
    claimBoundary: "Local pre-freeze product and research-harness verification. This proves only that the frozen candidate and selection machinery pass their declared local checks; it is not a Round 16 routing result or Agent A/B result.",
    competitionFreeze: {
      noCommit: true,
      noPush: true,
      noTag: true,
