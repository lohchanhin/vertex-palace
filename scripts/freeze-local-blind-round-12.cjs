...
const failedHeadEvidenceRelativePaths = [
  "docs/research/evidence/local-blind-routing-repository-head-verification-0.4-alpha-round-12.json"
];
const headEvidenceRelativePath = "docs/research/evidence/local-blind-routing-repository-head-verification-attempt-2-0.4-alpha-round-12.json";
const failedVerificationRelativePaths = [];
const verificationRelativePath = "docs/research/evidence/local-blind-routing-prefreeze-verification-0.4-alpha-round-12.json";
const round11FreezeRelativePath = "docs/research/evidence/local-blind-candidate-freeze-0.4-alpha-round-11.json";
const artifactPaths = [
  "docs/research/LOCAL_BLIND_ROUTING_TARGET_SELECTION_PROTOCOL_0_4_ALPHA_ROUND_12.md",
  "docs/zh-CN/LOCAL_BLIND_ROUTING_TARGET_SELECTION_PROTOCOL_0_4_ALPHA_ROUND_12.md",
  poolRelativePath,
  ...failedHeadEvidenceRelativePaths,
  headEvidenceRelativePath,
  ...failedVerificationRelativePaths,
  verificationRelativePath,
  "scripts/prepare-local-blind-routing-candidates-round-12.cjs",
  "scripts/finalize-local-blind-routing-targets-round-12.cjs",
  "scripts/verify-round12-repository-pool-heads.cjs",
  "scripts/verify-round12-prefreeze.cjs",
  "scripts/freeze-local-blind-round-12.cjs",
  ...
  "scripts/lib/round12-target-selection.cjs",
  "scripts/test/task-diff-coherence.test.cjs",
  "scripts/test/round12-task-diff-coherence-protocol.test.cjs",
  "scripts/test/round12-target-selection.test.cjs",
  "scripts/test/round12-candidate-queue.test.cjs",
  "scripts/test/round12-repository-pool.test.cjs",
  "scripts/test/round12-local-blind-selection-freeze.test.cjs",
  "docs/research/DISCLOSED_ROUTING_ROUND_11_FOCUS_REPAIR_RESULT_0_4_ALPHA.md",
  "docs/zh-CN/DISCLOSED_ROUTING_ROUND_11_FOCUS_REPAIR_RESULT_0_4_ALPHA.md",
  "docs/research/evidence/disclosed-routing-round-11-after-focus-repair-attempt-1-0.4-alpha.json",
  "scripts/verify-disclosed-routing-round-11-after-focus-repair.cjs",
  "scripts/test/round11-disclosed-focus-repair.test.cjs"
];
...
async function main() {
  ...
  const round11FreezeBytes = await readFile(path.join(projectRoot, round11FreezeRelativePath));
  const round11Freeze = JSON.parse(round11FreezeBytes.toString("utf8"));
  assert.equal(round11Freeze.comparisonBaseline.productCommit, baseCommit);
  run("git", ["cat-file", "-e", `${round11Freeze.comparisonBaseline.productCommit}^{commit}`]);
  ...
  const freeze = {
    ...
    claimBoundary: "Tamper-evident local freeze created after URL-and-HEAD-only pool verification and before any Round 12 repository history, subject, diff, task, coherence review, oracle, or Palace result was inspected. It is not public preregistration and cannot establish Agent correctness, Token savings, tool-call reduction, or wall-time improvement.",
    competitionFreeze: {
      noCommit: true,
      noPush: true,
      noTag: true,
    ...
    candidate: {
      role: "post-round-11-focus-repair-candidate",
      baseCommit,
    ...
    comparisonBaseline: {
      ...round11Freeze.comparisonBaseline,
      provenance: {
        path: round11FreezeRelativePath,
        sha256: sha256Bytes(round11FreezeBytes)
      }
    },
    preFreezeEvidence: {
      repositoryHeads: {
        ...
        failedAttempts: failedHeadEvidence.map(({ path: failedPath, bytes }) => ({
          ...
          category: "transient-github-connectivity-failure",
...
function outputArgument(args) {
  ...
  assert.equal(resolved, path.join(projectRoot, freezeRelativePath), "Round 12 freeze must use its canonical path");
  return resolved;
}
