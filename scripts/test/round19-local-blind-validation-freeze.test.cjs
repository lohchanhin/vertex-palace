...
const {
  ...
  normalizePath
} = require("../verify-local-blind-routing-round-19.cjs");

const root = path.resolve(__dirname, "../..");

test("Round 19 validation uses balanced order and frozen calibration", () => {
...
test("Round 19 absolute gate prioritizes evidence and safety over payload", () => {
  const passing = {
...
test("Round 19 validator is create-only, static-only, and hash bound", async () => {
  const validator = await readFile(path.join(root, "scripts/verify-local-blind-routing-round-19.cjs"), "utf8");
  const english = await readFile(
    path.join(root, "docs/research/LOCAL_BLIND_ROUTING_VALIDATION_PROTOCOL_0_4_ALPHA_ROUND_19.md"),
    "utf8"
  );
  const chinese = await readFile(
    path.join(root, "docs/zh-CN/LOCAL_BLIND_ROUTING_VALIDATION_PROTOCOL_0_4_ALPHA_ROUND_19.md"),
    "utf8"
  );
  const freeze = JSON.parse(await readFile(
    path.join(root, "docs/research/evidence/local-blind-routing-validation-freeze-0.4-alpha-round-19.json"),
    "utf8"
  ));
  ...
  assert.match(chinese, /不能补救缺失证据或不安全停止/);
  assert.equal(freeze.status, "locally-frozen");
  assert.equal(freeze.studyId, "local-blind-routing-validation-round-19-attempt-1-0.4-alpha");
  assert.equal(freeze.freezeAttempt, 1);
  ...
  assert.equal(freeze.preservation.productChangesForbiddenUntilFirstResult, true);
  assert.equal(freeze.competitionFreeze.noPush, true);
});

test("Round 19 validation freeze binds the coherent target manifest and review evidence", async () => {
  const freeze = JSON.parse(await readFile(
    path.join(root, "docs/research/evidence/local-blind-routing-validation-freeze-0.4-alpha-round-19.json"),
    "utf8"
  ));
  const manifestBytes = await readFile(
    path.join(root, "docs/research/evidence/local-blind-routing-target-manifest-0.4-alpha-round-19.json")
  );
  const candidateFreezeBytes = await readFile(
    path.join(root, "docs/research/evidence/local-blind-candidate-freeze-0.4-alpha-round-19.json")
  );
  ...
  assert.equal(freeze.inputs.coherenceReviewsSha256, manifest.semanticReview.sha256);
  assert.equal(freeze.inputs.candidateQueueSha256, manifest.candidateQueue.sha256);
});
