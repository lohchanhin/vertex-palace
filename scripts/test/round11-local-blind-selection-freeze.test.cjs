...
const freezePath = path.join(
  root,
  "docs/research/evidence/local-blind-candidate-freeze-0.4-alpha-round-11.json"
);

test("Round 11 candidate freeze binds the verified candidate and all selection machinery", async () => {
  ...
  const requiredArtifacts = [
    ...
    "docs/research/evidence/local-blind-routing-repository-head-verification-0.4-alpha-round-11.json",
    ...
    "docs/research/evidence/local-blind-routing-prefreeze-verification-attempt-3-0.4-alpha-round-11.json",
  ...
  assert.equal(freeze.candidate.role, "post-round-10-generic-causal-repair-candidate");
...
test("Round 11 freeze binds the 16-of-16 HEAD check and passing pre-freeze suite", async () => {
  ...
  assert.equal(freeze.preFreezeEvidence.failedAttempts.length, 2);
  for (const failedAttempt of freeze.preFreezeEvidence.failedAttempts) {
    const failedBytes = await readFile(path.join(root, failedAttempt.path));
    const failed = JSON.parse(failedBytes.toString("utf8"));
    assert.equal(sha256Bytes(failedBytes), failedAttempt.sha256);
    assert.equal(failed.status, "failed");
    assert.equal(failed.results[1].status, "execution-error");
    assert.equal(failedAttempt.productCommandsStarted, 0);
  }
});

test("Round 11 freeze preserves the competition and claim boundaries", async () => {
  const freeze = JSON.parse(await readFile(freezePath, "utf8"));
