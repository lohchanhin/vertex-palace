...
const {
  ...
  normalizePath
} = require("../verify-local-blind-routing-round-12.cjs");

const root = path.resolve(__dirname, "../..");

test("Round 12 validation uses balanced order and frozen calibration", () => {
...
test("Round 12 absolute gate prioritizes coverage and safety over payload", () => {
  const passing = {
...
test("Round 12 validator is create-only, static-only, and hash bound", async () => {
  const validator = await readFile(path.join(root, "scripts/verify-local-blind-routing-round-12.cjs"), "utf8");
  const english = await readFile(
    path.join(root, "docs/research/LOCAL_BLIND_ROUTING_VALIDATION_PROTOCOL_0_4_ALPHA_ROUND_12.md"),
    "utf8"
  );
  const chinese = await readFile(
    path.join(root, "docs/zh-CN/LOCAL_BLIND_ROUTING_VALIDATION_PROTOCOL_0_4_ALPHA_ROUND_12.md"),
    "utf8"
  );
  const freeze = JSON.parse(await readFile(
    path.join(root, "docs/research/evidence/local-blind-routing-validation-freeze-attempt-2-0.4-alpha-round-12.json"),
    "utf8"
  ));
  ...
  assert.match(chinese, /不能补救正确性、证据门或安全停止失败/);
  assert.equal(freeze.status, "locally-frozen");
  assert.equal(freeze.studyId, "local-blind-routing-validation-round-12-attempt-2-0.4-alpha");
  ...
  assert.equal(freeze.amendment.semanticProtocolChange, false);
  assert.equal(freeze.amendment.productChange, false);
});

test("Round 12 validation freeze binds the coherent target manifest", async () => {
  const freeze = JSON.parse(await readFile(
    path.join(root, "docs/research/evidence/local-blind-routing-validation-freeze-attempt-2-0.4-alpha-round-12.json"),
    "utf8"
  ));
  const manifestBytes = await readFile(
    path.join(root, "docs/research/evidence/local-blind-routing-target-manifest-0.4-alpha-round-12.json")
  );
  const candidateFreezeBytes = await readFile(
    path.join(root, "docs/research/evidence/local-blind-candidate-freeze-0.4-alpha-round-12.json")
  );
