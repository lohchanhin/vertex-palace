...
const {
  ...
  normalizePath
} = require("../verify-local-blind-routing-round-10.cjs");

const root = path.resolve(__dirname, "../..");

test("Round 10 validation uses balanced order and frozen calibration", () => {
...
test("Round 10 absolute gate prioritizes coverage and safety over payload", () => {
  const passing = {
...
test("Round 10 validator is create-only, static-only, and hash bound", async () => {
  const validator = await readFile(path.join(root, "scripts/verify-local-blind-routing-round-10.cjs"), "utf8");
  const english = await readFile(
    path.join(root, "docs/research/LOCAL_BLIND_ROUTING_VALIDATION_PROTOCOL_0_4_ALPHA_ROUND_10.md"),
    "utf8"
  );
  const chinese = await readFile(
    path.join(root, "docs/zh-CN/LOCAL_BLIND_ROUTING_VALIDATION_PROTOCOL_0_4_ALPHA_ROUND_10.md"),
    "utf8"
  );
  const freeze = JSON.parse(await readFile(
    path.join(root, "docs/research/evidence/local-blind-routing-validation-freeze-0.4-alpha-round-10.json"),
    "utf8"
  ));
  ...
  assert.equal(freeze.studyId, "local-blind-routing-validation-round-10-attempt-1-0.4-alpha");
...
test("Round 10 validation freeze binds the selected manifest without exposing it", async () => {
  const freeze = JSON.parse(await readFile(
    path.join(root, "docs/research/evidence/local-blind-routing-validation-freeze-0.4-alpha-round-10.json"),
    "utf8"
  ));
  const manifestBytes = await readFile(
    path.join(root, "docs/research/evidence/local-blind-routing-target-manifest-0.4-alpha-round-10.json")
  );
  const candidateFreezeBytes = await readFile(
    path.join(root, "docs/research/evidence/local-blind-candidate-freeze-0.4-alpha-round-10.json")
  );
