...
const harnessPath = path.join(projectRoot, "scripts", "verify-disclosed-routing-round-7-after-task-anchored-module-pair-repair.cjs");
...
const previousPath = evidencePath("disclosed-routing-round-7-after-task-morphology-repair-0.4-alpha.json");

test("freezes a disclosed module-pair regression with explicit claim limits", () => {
  ...
  assert.match(source, /repair:\s*"task-anchored-module-pair-v1"/);
...
test("locks both earlier Round 7 observations before measurement", () => {
  ...
  assert.equal(
    sha256(previousPath),
    "9779EBEC4A235008DF42B915073B87E93079CF8168B7FAFCA2D42C9CE439BF71"
  );
  ...
  assert.equal(previous.candidate.productCommit, "084aae31900e41952bc90aaaf959647c473bcc43");
