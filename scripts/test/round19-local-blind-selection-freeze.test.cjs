...
test("Round 19 candidate freeze binds the candidate and all selection machinery", async () => {
  ...
  const requiredArtifacts = [
    ...
    "docs/research/evidence/local-blind-routing-prefreeze-verification-0.4-alpha-round-19.json",
...
test("Round 19 freeze binds the 48-entry reachability roster and 32-repository pool", async () => {
  ...
  assert.equal(headEvidence.repositories.filter(({ status }) => status === "definitively-missing").length, 3);
...
test("Round 19 freeze preserves failed-pool lineage, competition freeze, and claim limits", async () => {
  const freeze = JSON.parse(await readFile(freezePath, "utf8"));
  assert.equal(freeze.preFreezeEvidence.priorPoolFailures.length, 2);
  for (const priorFailure of freeze.preFreezeEvidence.priorPoolFailures) {
    const bytes = await readFile(path.join(root, priorFailure.path));
    const evidence = JSON.parse(bytes.toString("utf8"));
    assert.equal(sha256Bytes(bytes), priorFailure.sha256);
