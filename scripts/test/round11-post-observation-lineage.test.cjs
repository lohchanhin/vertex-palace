...
test("preserves the Round 11 frozen candidate lineage after disclosed product development", async () => {
  const freezePath = path.join(evidenceRoot, "local-blind-candidate-freeze-0.4-alpha-round-11.json");
  ...
  const liveGuardPath = path.join(root, "scripts", "test", "round11-local-blind-selection-freeze.test.cjs");
  ...
  assert.equal(
    ...
    "The formal result no longer binds the original candidate freeze"
  );
  assert.equal(
    sha256(manifestBytes),
    formal.frozenInputs.manifestSha256,
    "The formal result no longer binds the original target manifest"
  );
  assert.equal(
    sha256(liveGuardBytes),
    freeze.artifacts["scripts/test/round11-local-blind-selection-freeze.test.cjs"],
    "The pre-observation live guard changed after the freeze"
  );
...
test("keeps the regular research runner explicit about the retired live guard", async () => {
  ...
  assert.match(runner, /round11-local-blind-selection-freeze\.test\.cjs/);
  assert.match(runner, /pre-observation live candidate guard is preserved/i);
  assert.match(runner, /--test-name-pattern/);
  assert.match(runner, /Round 11 freeze/);
});
