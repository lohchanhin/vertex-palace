...
test("Round 21 preregisters a fresh balanced roster before remote HEAD queries", async () => {
  const plan = JSON.parse(await readFile(planPath, "utf8"));
  assert.equal(plan.studyId, "local-blind-routing-round-21-0.4-stable-candidate");
  assert.equal(plan.status, "preregistered-before-head-query");
  assert.equal(plan.publicPreregistration, true);
  assert.equal(plan.products.candidate.package, "vertex-palace@0.4.0");
  assert.equal(plan.products.candidate.npmDistTagAtPreregistration, null);
...
test("Round 21 freezes the stable release decision before observation", async () => {
  const protocol = await readFile(
    path.join(root, "docs/research/LOCAL_BLIND_ROUTING_VALIDATION_PROTOCOL_0_4_STABLE_ROUND_21.md"),
    "utf8"
  );
  ...
  assert.match(protocol, /If the Round 21 gate fails, `0\.4\.0` is not published/);
  assert.match(protocol, /`0\.4\.0-alpha\.1` remains on npm `next`/);
