...
const englishPath = path.join(root, "docs", "research", "LOCAL_BLIND_ROUTING_TARGET_SELECTION_PROTOCOL_0_4_ALPHA_ROUND_12.md");
const chinesePath = path.join(root, "docs", "zh-CN", "LOCAL_BLIND_ROUTING_TARGET_SELECTION_PROTOCOL_0_4_ALPHA_ROUND_12.md");
const libraryPath = path.join(root, "scripts", "lib", "task-diff-coherence.cjs");

test("keeps Round 12 explicitly unexecuted until a matching candidate freeze", async () => {
  const [english, chinese] = await Promise.all([
    readFile(englishPath, "utf8"),
    readFile(chinesePath, "utf8")
  ]);
  assert.match(english, /target selection has not started/i);
  ...
  assert.match(english, /No Round 12 repository history, candidate task, diff/i);
  assert.match(english, /not public preregistration/i);
  assert.match(chinese, /第 12 轮/);
...
test("locks whole-target semantic review before any Palace call", async () => {
  ...
  assert.match(english, /Palace calls on candidate tasks must remain zero until review finalization/);
  assert.match(english, /one developer-delegated semantic reviewer.*machine completeness validation/i);
  assert.match(english, /no second independent human reviewer or inter-rater agreement is available/i);
...
test("preserves the absolute static gate and fresh Round 12 advancement rule", async () => {
  const english = await readFile(englishPath, "utf8");
  ...
  assert.match(english, /Only a valid fresh Round 12 absolute pass/);
});
