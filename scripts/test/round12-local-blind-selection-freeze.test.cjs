...
const freezePath = path.join(
  root,
  "docs/research/evidence/local-blind-candidate-freeze-0.4-alpha-round-12.json"
);

test("Round 12 candidate freeze binds the verified candidate and all selection machinery", async () => {
  ...
  const requiredArtifacts = [
    ...
    "docs/research/evidence/local-blind-routing-repository-head-verification-attempt-2-0.4-alpha-round-12.json",
    "docs/research/evidence/local-blind-routing-prefreeze-verification-0.4-alpha-round-12.json",
  ...
  assert.equal(freeze.candidate.role, "post-round-11-focus-repair-candidate");
...
test("Round 12 freeze binds the 16-of-16 HEAD check and passing pre-freeze suite", async () => {
  ...
  assert.ok(headEvidence.repositories.every(({ matched }) => matched));
  assert.equal(headEvidence.commitHistoryInspected, false);
  assert.equal(freeze.preFreezeEvidence.repositoryHeads.failedAttempts.length, 1);
  const [failedHeadAttempt] = freeze.preFreezeEvidence.repositoryHeads.failedAttempts;
  ...
  assert.equal(failedHeadAttempt.category, "transient-github-connectivity-failure");
...
test("Round 12 freeze preserves the competition and claim boundaries", async () => {
  const freeze = JSON.parse(await readFile(freezePath, "utf8"));
