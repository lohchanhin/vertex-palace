...
async function main() {
  ...
  assert.equal(
    ...
    "Prior exclusion source changed after the Round 17 plan was written"
  );
  ...
  assert.equal(
    ...
    "Round 17 prior-observation count differs from the frozen plan"
  );

  const plannedUrls = plan.repositoryPlan.map(({ url }) => normalizeRepositoryUrl(url));
  assert.equal(new Set(plannedUrls).size, plannedUrls.length, "Round 17 plan contains duplicate URLs");
  for (const repository of plan.repositoryPlan) {
    assert.ok(
      !observed.has(normalizeRepositoryUrl(repository.url)),
      `${repository.name} was already observed before Round 17`
    );
  }
  ...
  const pool = {
    ...
    claimBoundary: "Fresh URL-and-HEAD-only repository pool frozen before any Round 17 repository history, commit subject, diff, task, coherence review, oracle, or Palace result was inspected. The eight-repository fallback quota per family was fixed after the preserved Round 16 mechanical-feasibility failure and before any Round 17 HEAD query. Candidate source, selectors, and validation artifacts must still be frozen before target selection. This is not public preregistration.",
    candidateFreeze: {
    ...
    priorExclusionSource: {
      ...
      includeRound16RepositoryPool: true,
...
function assertPlan(plan) {
  ...
  assert.equal(plan.priorExclusionSource.expectedUniqueRepositoryCount, 178);
