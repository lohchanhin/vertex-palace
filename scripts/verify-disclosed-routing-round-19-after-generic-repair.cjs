...
const formalRelativePath = "docs/research/evidence/local-blind-routing-validation-0.4-alpha-round-19-attempt-2.json";
...
const studyId = outputPath
  ...
  : "disclosed-routing-round-19-after-generic-repair-attempt-1-0.4-alpha";
const repetitions = 2;
const budget = 6_000;
const routeLimit = 10;
const maxDrawers = 4;
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-round-19-disclosed-repair-"));
  const targets = [];

  try {
    for (const target of formal.targets) {
      const root = path.join(temporaryRoot, safeSegment(target.name));
      const formalPair = formal.comparison.targetPairs.find((entry) => entry.target === target.name);
      assert.ok(formalPair, `Formal target comparison is missing: ${target.name}`);
      try {
        await materializeTarget(target, root);
        const repairedCandidate = validateTarget(target, root);
        targets.push({
          ...publicTargetIdentity(target),
          formalCandidate: formalPair.candidate,
          repairedCandidate,
          comparison: compareTarget(formalPair.candidate, repairedCandidate)
        });
    ...
    assert.equal(await sha256File(formalPath), formalSha256, "Formal Round 19 evidence changed during replay");
    ...
    const report = {
      ...
      claimBoundary: "Disclosed post-observation replay on the eight already observed Round 19 tasks after repository-generic lexical morphology, package-entry import, package-root verification, long-symbol compound-evidence, task-module identity, and diagnostic causal-closure repairs. The immutable formal Round 19 candidate gate remains failed. This replay does not execute target tests or an Agent and supports no Agent correctness, Token, tool-call, or wall-time claim. Fresh frozen repositories are required for new generalization evidence.",
      formalEvidence: {
...
function compareTarget(formalCandidate, repairedCandidate) {
  ...
  return {
    routeChanged: formalCandidate.routeFiles !== current.routeFiles.join(" "),
...
function outputArgument(args) {
  ...
  assert.ok(index >= 0 && args[index + 1], "Usage: node scripts/verify-disclosed-routing-round-19-after-generic-repair.cjs --output <new-json-path>");
