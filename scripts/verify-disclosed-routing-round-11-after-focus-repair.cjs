...
const originalRelativePath = "docs/research/evidence/local-blind-routing-validation-0.4-alpha-round-11-attempt-1.json";
const previousRepairRelativePath = "docs/research/evidence/disclosed-routing-round-11-after-owner-closure-repair-attempt-3-0.4-alpha.json";
...
const previousRepairPath = path.join(projectRoot, previousRepairRelativePath);
const outputPath = require.main === module ? outputArgument(process.argv.slice(2)) : null;
const studyId = "disclosed-routing-round-11-after-focus-repair-0.4-alpha";
const targetCount = 8;
const repetitions = 2;
const budget = 6_000;
const routeLimit = 10;
...
async function main() {
  const [manifest, original, previousRepair] = await Promise.all([
    readJson(manifestPath),
    readJson(originalPath),
    readJson(previousRepairPath)
  ]);
  assert.equal(manifest.targets.length, targetCount);
  assert.equal(original.candidateGateStatus, "failed");
  assert.equal(original.status, "completed");
  assert.equal(previousRepair.gateStatus, "failed");
  assert.deepEqual(previousRepair.gateFailures, ["target-macro route focus below threshold"]);
  assert.equal(previousRepair.aggregate.coreSurfaceCompleteTargets, targetCount);

  const originalSha256 = await sha256File(originalPath);
  const manifestSha256 = await sha256File(manifestPath);
  const previousRepairSha256 = await sha256File(previousRepairPath);
  const candidateCliSha256 = await sha256File(candidateCliPath);
  const candidateSourceTreeBefore = await hashSourceTree();
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-round-11-disclosed-focus-repair-"));
  const targets = [];
  ...
  try {
    for (const target of manifest.targets) {
      ...
      const previousRepairTarget = previousRepair.targets.find((entry) => entry.name === target.name);
      assert.ok(originalTarget, `Original Round 11 target is missing: ${target.name}`);
      assert.ok(previousRepairTarget, `Previous disclosed repair target is missing: ${target.name}`);
      ...
      try {
        ...
        targets.push({
          ...
          previousDisclosedCandidate: compactRepairedCandidate(previousRepairTarget),
          repairedCandidate: result,
          comparison: compareTarget(originalTarget, result),
          comparisonWithPreviousRepair: compareRepairedTarget(previousRepairTarget, result)
        });
    ...
    const previousRepairSha256After = await sha256File(previousRepairPath);
    ...
    assert.equal(previousRepairSha256After, previousRepairSha256);
    ...
    const report = {
      ...
      claimBoundary: "Disclosed regression on the eight already observed Round 11 tasks after generic runtime-test evidence budgeting, runtime/type owner de-duplication, verification-configuration intent gating, and weak lexical metadata pruning. The immutable formal Round 11 candidate gate and prior owner-closure attempt remain historical evidence. This result is not held out, does not execute target tests or an Agent, and supports no Agent correctness, Token, tool-call, or wall-time claim. A fresh Round 12 is required for new generalization evidence.",
      ...
      previousDisclosedRepair: {
        path: previousRepairRelativePath,
        sha256: previousRepairSha256,
        gateStatus: previousRepair.gateStatus,
        gateFailures: previousRepair.gateFailures,
        targetMacroRouteFocus: previousRepair.aggregate.targetMacroRouteFocus,
        preservedWithoutModification: true
      },
      comparisonWithPreviousDisclosedRepair: {
        targetMacroChangedFileCoverageDelta: round(
          aggregate.targetMacroChangedFileCoverage
            - previousRepair.aggregate.targetMacroChangedFileCoverage
        ),
        targetMacroCoreSurfaceCoverageDelta: round(
          aggregate.targetMacroCoreSurfaceCoverage
            - previousRepair.aggregate.targetMacroCoreSurfaceCoverage
        ),
        targetMacroRouteFocusDelta: round(
          aggregate.targetMacroRouteFocus
            - previousRepair.aggregate.targetMacroRouteFocus
        ),
        targetMacroCoreRouteFocusDelta: round(
          aggregate.targetMacroCoreRouteFocus
            - previousRepair.aggregate.targetMacroCoreRouteFocus
        )
      },
      targetSelection: {
      ...
      candidate: {
        ...
        sourceState: "local uncommitted disclosed focus-repair worktree",
      ...
      protocol: {
        ...
        oraclePolicy: {
          ...
          fullChangedFiles: "Retained for comparability with the immutable formal Round 11 result."
        },
        gates: {
...
function compactRepairedCandidate(target) {
  const trial = target.repairedCandidate?.trials?.[0] ?? null;
  return {
    conditionStatus: target.repairedCandidate?.status ?? null,
    failures: target.repairedCandidate?.failures ?? [],
    trial: trial ? {
      routeFiles: trial.routeFiles,
      changedFileCoverage: trial.changedFileCoverage,
      coreSurfaceCoverage: trial.coreSurfaceCoverage,
      ...
      mode: trial.mode,
      evidenceStatus: trial.evidenceStatus,
      unsafeNarrowAgainstCore: trial.unsafeNarrowAgainstCore
    } : null
  };
}

function compareRepairedTarget(previousTarget, repaired) {
  const before = compactRepairedCandidate(previousTarget).trial;
  const after = repaired.trials[0];
  if (!before || !after) return null;
  return {
    routeChanged: !sameValues(before.routeFiles, after.routeFiles),
