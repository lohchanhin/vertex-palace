...
const baselineRelativePath = "docs/research/evidence/held-out-cross-repository-routing-0.4-alpha-round-7.json";
...
const studyId = "disclosed-routing-round-7-after-task-morphology-repair-0.4-alpha";
const candidateCommit = "084aae31900e41952bc90aaaf959647c473bcc43";
const candidateCliSha256 = "0B26DF95458F7302FB76EFB3BD91929EEDEA1F5B94E995339F3D4190141C5C51";
...
const baselineSha256 = "C53C9186853F1661158212604804261C8394781454A7C7B7DF15811D4E42D7B9";
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(
    path.join(os.tmpdir(), "vertex-palace-disclosed-routing-round-7-task-morphology-")
  );
  ...
  try {
    ...
    report = {
      ...
      failures,
      claimBoundary: "Post-failure static routing regression on the eight disclosed Round 7 tasks after only the generic task-action morphology repair. It can attribute changes on known failures to that repair, but cannot establish held-out generalization or end-to-end Agent correctness, Token, tool-call, or wall-time gains.",
      heldOutAgainstCandidate: false,
      evidenceClass: "seen-development-regression",
      candidate: {
        productCommit: candidateCommit,
        repair: "task-action-morphology-v1",
    ...
    if (process.env.KEEP_DISCLOSED_ROUTING_ROUND_7_TASK_MORPHOLOGY_TEMP === "1") {
...
async function assertFrozenInputs() {
  ...
  assert.equal(baseline.candidate.productCommit, originalCandidateCommit);
...
function outputArgument(args) {
  ...
  assert.notEqual(resolved, baselinePath, "Output cannot overwrite the original Round 7 held-out observation.");
