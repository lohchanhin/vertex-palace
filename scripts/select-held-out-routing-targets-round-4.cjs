...
const poolRelativePath = "docs/research/evidence/held-out-routing-repository-pool-0.4-alpha-round-4.json";
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-held-out-round-4-selection-"));
  ...
  try {
    ...
    manifest = {
      ...
      selector: {
        ...
        path: "scripts/select-held-out-routing-targets-round-4.cjs"
      },
      repositoryPool: {
    ...
    if (process.env.KEEP_HELD_OUT_SELECTION_ROUND_4_TEMP === "1") {
      process.stderr.write(`Keeping Round 4 selection repositories at ${temporaryRoot}\n`);
    } else {
