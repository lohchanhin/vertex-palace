...
const poolRelativePath = "docs/research/evidence/held-out-routing-repository-pool-0.4-alpha-round-7.json";
...
async function main() {
  ...
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "vertex-palace-held-out-round-7-selection-"));
  ...
  try {
    ...
    manifest = {
      ...
      selector: {
        ...
        path: "scripts/select-held-out-routing-targets-round-7.cjs"
      },
      repositoryPool: {
    ...
    if (process.env.KEEP_HELD_OUT_SELECTION_ROUND_7_TEMP === "1") {
      process.stderr.write(`Keeping Round 7 selection repositories at ${temporaryRoot}\n`);
    } else {
