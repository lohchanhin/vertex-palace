...
比赛结果冻结期禁止 commit 与 push，因此这属于内部防篡改盲测，不是公开预注册。
它可以决定研发是否进入新的 Agent 实验，但本身不能证明外部泛化。

## 已保存的验证 preflight 第 1 次尝试

第一次启动在 materialize 目标以前、也在任何选中任务进入 Palace 以前停止。
Windows 上的 Node 直接启动 `pnpm.cmd` 时返回 `EINVAL`，发生位置是离线重建基线。
create-only 无效结果已保存在
...
第 2 次只把 Windows pnpm 启动方式改成既有研究脚本已经使用的
`cmd.exe /d /s /c` 包装。产品、目标 manifest、任务、oracle、条件顺序、指标、
门槛、重试规则与保存规则都不改变；修订后的 validator 与协议会再次完成本地
哈希冻结，并写入另一个 create-only 结果路径。
...
因为即使平衡条件顺序，文件系统与作业系统 cache 仍无法完全消除。
...
若进行修复，第 9 轮全部任务都只能当作 disclosed regression cases。

## 证据保存

Preflight 第 1 次尝试已经建立并保留下列不可变的无效结果：

`docs/research/evidence/local-blind-routing-validation-0.4-alpha-round-9.json`

修订后的 validator 只建立一次以下独立的 create-only 正式结果：
