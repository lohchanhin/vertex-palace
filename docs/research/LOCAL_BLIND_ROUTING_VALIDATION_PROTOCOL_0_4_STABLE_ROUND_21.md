# Local Blind Paired Routing Validation Protocol (0.4 Stable Candidate, Round 21)

## Status and Evidence Boundary

This protocol, final-version source, and ordered 48-repository roster are committed and pushed before any Round 21 remote HEAD query, repository history inspection, target selection, or Palace call on a candidate task. The candidate is the publicly committed but not-yet-published `vertex-palace@0.4.0` tarball; the comparison baseline is the immutable public package `vertex-palace@0.3.0`. The exact candidate CLI, MCP bundle, source tree, and tarball hashes are frozen before target history is inspected.

Round 20 and its post-observation repairs remain disclosed evidence and are excluded from target selection. Round 21 is a fresh static-routing study. It does not execute an Agent or target test suite and cannot by itself prove end-to-end correctness, Token savings, tool-call reduction, or wall-time improvement.

## Research Question

On eight previously unobserved real-repository tasks, does the frozen 0.4 stable candidate preserve or improve 0.3 routing coverage and focus while eliminating unsafe narrowing, keeping context bounded, and reporting calibrated confidence?
...
1. Round 21 is valid and the candidate absolute gate passes without changing targets, oracles, thresholds, or product code;
...
If the Round 21 gate fails, `0.4.0` is not published, `0.4.0-alpha.1` remains on npm `next`, `0.3.0` remains `latest`, and the negative result is published unchanged. Post-observation repair may begin only after that result is preserved and cannot retroactively qualify this round.
