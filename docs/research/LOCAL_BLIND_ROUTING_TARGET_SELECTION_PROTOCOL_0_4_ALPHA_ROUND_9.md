# Local Blind Routing Target Selection Protocol (0.4 Alpha, Round 9)

## Status and Claim Boundary

This protocol was locally frozen before inspecting any Round 9 repository
...
## Preserved Attempt 1 and Infrastructure Amendment

Selection attempt 1 stopped while inspecting the frozen `rand` repository. A
GitHub DNS failure occurred when the partial clone tried to fetch a missing
promisor object. No target task was printed or sent to Palace, no target
manifest was produced, and the product remained byte-identical to the freeze.
The failed attempt is preserved in
`docs/research/evidence/local-blind-routing-target-selection-round-9-attempt-1-failure.json`.

Before attempt 2, the harness was amended only to fetch the complete frozen
shallow history, catch per-repository inspection errors, and preserve unexpected
selector failures as create-only evidence. The product, repository pool, order,
eligibility rules, task classifier, quotas, and metrics did not change. The
amended artifacts received a new local hash freeze before retrying selection.

## Research Question

Before paying for another end-to-end Agent experiment, does the current local
0.4 candidate recover the implementation, focused-test, and bounded auxiliary
evidence for new real-repository tasks more reliably than the pre-optimization
baseline, while keeping routes focused and delivered context bounded?
...
changed after this freeze until the first Round 9 result is preserved.

## Complete Repository Exclusion

The exclusion set is derived mechanically from the frozen Round 8 pool:

- every repository previously observed before Round 8; and
- all sixteen Round 8 primary and fallback repositories, whether inspected or
  not.

The expected union contains 97 unique repository URLs. Round 9 contains sixteen
...
Round 9 selects eight targets, exactly two each from JavaScript/TypeScript,
...
non-merge commits wins. Eligibility is unchanged from Round 8:
