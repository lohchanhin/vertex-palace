...
**Stable release:** Version `0.4.0` is the current stable line. It makes Palace advisory by default, adds evidence-role and dependency-closure routing, protects structured decision memory, reports exact section-level UTF-8 costs, and delivers explicit no-reopen and stop boundaries. Install it with `npm install -g vertex-palace@latest`, or add the plugin marketplace with `codex plugin marketplace add lohchanhin/vertex-palace --ref v0.4.0`.

The preserved Round 20 stable gate failed before the final generic repairs. Its post-observation replay later reached 8/8 known targets, but that result was not treated as held-out proof. The stable release decision is bound to the fresh Round 21 protocol and result. Static routing evidence does not establish a general reduction in end-to-end Agent tokens, wall time, or correctness errors. Read the [Round 20 result](./docs/research/LOCAL_BLIND_ROUTING_ROUND_20_RESULT_0_4_STABLE.md), [disclosed repair result](./docs/research/DISCLOSED_ROUTING_ROUND_20_GENERIC_REPAIR_RESULT_0_4_ALPHA.md), and [Round 21 protocol](./docs/research/LOCAL_BLIND_ROUTING_VALIDATION_PROTOCOL_0_4_STABLE_ROUND_21.md).

**稳定版本：** `0.4.0` 是当前稳定系列。这个版本默认把 Palace 作为建议层，新增证据角色与依赖闭包路由、结构化决策记忆、精确 UTF-8 分区成本，以及明确的禁止重开与停止边界。可运行 `npm install -g vertex-palace@latest`，或执行 `codex plugin marketplace add lohchanhin/vertex-palace --ref v0.4.0` 安装插件。

正式 Round 20 稳定版门禁在最终通用修复前失败，该结果仍完整保留。事后回放后来达到 8/8 已知目标，但没有被冒充为 held-out 证据。稳定版发布决定绑定到全新的 Round 21 协议与结果。静态路由证据不能证明 Agent 普遍减少总 Token、缩短时间或降低正确性错误。详见[第 20 轮结果](./docs/zh-CN/LOCAL_BLIND_ROUTING_ROUND_20_RESULT_0_4_STABLE.md)、[事后修复结果](./docs/zh-CN/DISCLOSED_ROUTING_ROUND_20_GENERIC_REPAIR_RESULT_0_4_ALPHA.md)与[第 21 轮协议](./docs/zh-CN/LOCAL_BLIND_ROUTING_VALIDATION_PROTOCOL_0_4_STABLE_ROUND_21.md)。
...
A first non-formal Agent preflight now separates product-contract success from end-to-end behavior. On the small local task, Adaptive delivered a 177-byte bypass but still used 6 more tool calls, 14,611 more reported tokens, and 5.5 more seconds than Control because later exploration expanded again. On the memory-dependent task, Adaptive delivered 2/2 relevant memories, selected the correct Aurora scope, and was descriptively lighter than Control, but Control also succeeded; no correctness advantage over Control is claimed. The bilingual [Agent preflight record](./docs/research/AGENT_PREFLIGHT_0_3_0.md) preserves both results and the measurement defects found along the way.

After that negative preflight, the final candidate removed package-command discovery and added bounded execution guidance. In two fresh opposite-order small-local pairs, all eight Arms were valid, correct, and strictly scoped. Adaptive inspected one file in both pairs and had paired medians of 17,316.5 fewer reported tokens and 9.445 seconds less wall time, while still using one more tool call. The public [revised confirmation report](https://github.com/lohchanhin/benchmarks-demo/blob/main/docs/research/REVISED_BYPASS_CONFIRMATION.md) retains the exact artifact hashes, the Control router-error caveat, and the reason this remains small-local exploratory evidence rather than a universal speed claim.
...
对于非 bypass 模式，0.3.0 把 `maxContextTokens` 当成实际交付契约：完整序列化 Markdown 或 JSON 后再测量，其中包括记忆筛选 telemetry 与执行边界；若超出上限，就先把低优先级源码降为摘要，再延后到 Deferred。干净安装测试使用 50 条相关记忆候选，JSON 实际交付 4,025 / 5,000 estimated tokens，Markdown 为 4,484 / 5,000；报告字节与真实输出完全一致，50 条被考虑的记忆都有纳入或排除原因。详见双语的 [context ceiling 事故与修复记录](./docs/research/ADAPTIVE_CONTEXT_CEILING_0_3_0.md)和[最终机器可读安装包证据](./docs/research/evidence/release-candidate-0.3.0-final-command.json)。这属于产品契约验证，不代表端到端 Agent 已经更省 Token。

0.3.0 新增真正的 Adaptive Bypass 与明确执行边界。任务不必先写出文件名：路由必须高信心只找到一个 Primary 文件，轻量记忆预检必须确认没有相关且仍有效的历史记录，同时不能有跨层、公共契约、客户隔离或广范围风险。Bypass 实际交付内容只有 `mode`、`primaryCandidate` 与 `reason`。其他模式会输出 Primary、Support、Deferred、Excluded、Required Evidence、Do Not、Stop Condition、Conflict Summary 与 Context 上限，告诉 Agent 哪些证据必须看、哪些区域不要碰，以及何时停止继续搜索。

公开 npm tarball 的 [Zod 与 Requests 固定仓库验证](./docs/research/REAL_REPOSITORY_VALIDATION_0_3_0.md)各重复两次，均只返回已知实现与对应测试两个文件，目标召回率与严格目标精度都为 `1.000`。这只是一项打包与路由门槛，不代表端到端 Agent 已经更快或更省 Token；宽范围任务的跨表面覆盖仍需继续改进。
...
第一轮非正式 Agent 预跑已经把产品契约通过与端到端行为分开。小型单文件任务中，Adaptive 虽然只交付 177 bytes 的 bypass，却因为后续重新扩大探索，相对 Control 多 6 次工具调用、多 14,611 reported tokens，并慢 5.5 秒。记忆依赖任务中，Adaptive 纳入 2/2 条相关记忆并正确选择 Aurora，单次资源使用也较低，但 Control 同样正确，因此尚不能宣称对 Control 有正确性优势。双语的 [Agent 预跑记录](./docs/research/AGENT_PREFLIGHT_0_3_0.md)保留了正反结果与本轮发现的测量缺陷。

针对这项负面结果，最终候选移除了 package 命令探索并加入有边界的执行指引。两组全新、反向顺序的 small-local 配对中，8 个 Arm 全部有效、正确且严格符合修改范围；Adaptive 两次都只检索一个文件，配对中位数少 17,316.5 reported tokens、快 9.445 秒，但仍多 1 次工具调用。公开的[修订版确认报告](https://github.com/lohchanhin/benchmarks-demo/blob/main/docs/zh-CN/REVISED_BYPASS_CONFIRMATION.md)同时保留 artifact 哈希、Control router error 限制与结论边界；这仍是 small-local 探索性证据，不是所有任务的普遍加速承诺。
