...
5. Inspect Primary first, then the named Required Evidence. Treat Support as bounded backup context. A drawer with `do_not_reopen: true`, `full_file`, or `full_symbol` is already delivered source: do not reopen that path. Batch-read only Required Evidence that was not delivered in full.
...
8. For cross-stack work, preserve the Contract Capsule input, output, invariant, and prohibited change across the single routed anchor for each layer. Run targeted and broader verification in one shell invocation when both are required. Combine final status and diff checks instead of spending separate calls on each, then obey `stopEnforced` and stop immediately when tests and changed-file scope pass.
...
- Do not reopen source marked `do_not_reopen: true`, and do not add a second implementation anchor for the same cross-stack layer unless current code, tests, or the oracle proves it is required.
