...
import type { PalaceEdge, PalaceEvidenceFact, PalaceIndex, PalaceNode, PalaceRoom, PalaceRoute } from "@vertex-palace/shared";
...
export async function readIndex(root: string): Promise<PalaceIndex> {
  ...
  const facts = await readJson<PalaceEvidenceFact[]>(root, ".palace/indexes/facts.json", []);
  ...
  return { nodes, edges, facts, rooms, symbols, directoryTree, fileHashes, routes };
}
